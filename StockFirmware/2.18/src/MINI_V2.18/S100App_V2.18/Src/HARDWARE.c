...
void Scan_Key(void)
{
    ...
    if(device_info.handers)//左手模式，按键是反过来的
    {
        if((~GPIOA->IDR) & 0x0200) key_state |= KEY_V2; //KEY_V1
        if((~GPIOA->IDR) & 0x0040) key_state |= KEY_V1; //KEY_V2
    }
    else
    {
    ...
    if(gLongkey_flag == 1) { //LongKey_flag :限制长按健标志
        if(key_statuslast == key_state) {
            p_cnt++;
            if(p_cnt > 25)    Set_gKey(KEY_CN | key_state);//长按键
        } else {
            p_cnt = 0;
...
void Key_Read( void )
{
    ...
    if(buttonA_cnt >= SI_DATA && buttonB_cnt >= SI_DATA && gLongkey_flag) Set_gKey(KEY_CN | KEY_V3);
        ...
        if(buttonA_cnt >= SI_DATA && gLongkey_flag && buttonB_cnt == 0) 
        {
            if(device_info.handers)     Set_gKey(KEY_CN | KEY_V2);
            else                        Set_gKey(KEY_CN | KEY_V1);
        }
        if(buttonB_cnt >= SI_DATA && gLongkey_flag && buttonA_cnt == 0) 
        {
