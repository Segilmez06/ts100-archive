...
2015/07/07   统一命名;
2017/04/01   取消单独的温度计模式新增设置模式原温度计模式功能也放入设置模式;
*******************************************************************************/
#include <stdio.h>
#include <string.h>
...
void SetOpt_Detailed(void)
{
  ...
  /*----------------范围和增加单位赋值-------------------*/
    ...
    /*------------------进入选项后的操作-------------------*/
        if(Get_gKey() != 0)
        {
            if(Get_gKey() == (KEY_CN | KEY_V3))//双键同时按，退出设置
            {
                EFFECTIVE_KEY_TIMER = 0;
            }
            /*--------------------减--------------------*/
            else if((Get_gKey() == (KEY_CN | KEY_V1)) || (Get_gKey() == KEY_V1))
            {
            ...
            /*--------------------加--------------------*/       
            else if((Get_gKey() == KEY_V2) || (Get_gKey() == (KEY_CN | KEY_V2)))
            {
...
void TempSet_Proc(void)
{
    ...
    if(EFFECTIVE_KEY_TIMER > 0) {
        ...
        }else if(Get_gKey() == KEY_V1 || Get_gKey() == (KEY_CN | KEY_V1))//减
        {
            if(Get_gKey() == KEY_V1)//单击
            {
                add_step = 0;
            }
            else //长按
        ...
        else if(Get_gKey() == KEY_V2 || Get_gKey() == (KEY_CN | KEY_V2))//加
        {
            if(Get_gKey() == KEY_V2)//单击
            {
                add_step = 0;
            }
            else//长按
